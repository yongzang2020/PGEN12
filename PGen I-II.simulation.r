...
biv_PGen12_g=function(ttox_g,teff_g,sigma12_g){#Function for a specific subgroup
...
biv_PGen12=function(ttox,teff,sigma12){#Function for a specific subgroup
...
uti_PGen12_g=function(ttox_g,teff_g,sigma12_g,score){#Function for a specific subgroup
...
uti_PGen12=function(ttox,teff,sigma12,score){#Function for a specific subgroup
...
get.boundary <- function(target, targetE, ncohort, cohortsize, p.saf=NA, p.tox=NA,  cutoff.eli, cutoff.eli.E){#Calculate the boundaries of the BOIN-12 design
...
pfs.gen_PGen12_weibull=function(omega,psi,gammawt,gammawe,gammawd,d,ye,yt){
  gammawd<-c(0,gammawd)
  ha=psi*(exp(gammawe*(ye==2)+gammawt*(yt==1)+gammawd[d]))^(-1/omega)
  return(rweibull(1,omega,ha))
}

gammawd_generate_weibull=function(pi,omega,gammawe,gammawt,sur.p){
  psi<-rep(0,3)
  gammawd<-matrix(0,3,3)
  for(i in 1:3){
    Fun1<-function(x){
      return(pi[1,2,1,i]*(1-pweibull(5,omega[i],x*(exp(gammawe[i]*(1==2)+gammawt[i]*(0==1)))^(-1/omega[i])))+
               pi[1,3,1,i]*(1-pweibull(5,omega[i],x*(exp(gammawe[i]*(2==2)+gammawt[i]*(0==1)))^(-1/omega[i])))+
               pi[2,2,1,i]*(1-pweibull(5,omega[i],x*(exp(gammawe[i]*(1==2)+gammawt[i]*(1==1)))^(-1/omega[i])))+
               pi[2,3,1,i]*(1-pweibull(5,omega[i],x*(exp(gammawe[i]*(2==2)+gammawt[i]*(1==1)))^(-1/omega[i])))-sur.p[i,1])
    }
    psi[i]<-as.numeric(uniroot(Fun1,c(1e-15,100))[1])
    for(j in 2:4){
      Fun2<-function(x){
        return(pi[1,2,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(1==2)+gammawt[i]*(0==1)+x))^(-1/omega[i])))+
                 pi[1,3,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(2==2)+gammawt[i]*(0==1)+x))^(-1/omega[i])))+
                 pi[2,2,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(1==2)+gammawt[i]*(1==1)+x))^(-1/omega[i])))+
                 pi[2,3,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(2==2)+gammawt[i]*(1==1)+x))^(-1/omega[i])))-sur.p[i,j])
      }
...
sur.p_PGen12_weibull=function(pi,omega,psi,gammawe,gammawt,gammawd){
  gammawd<-rbind(rep(0,3),gammawd)
  re<-matrix(0,3,4)
  for(i in 1:3){
    for(j in 1:4){
      re[i,j]<-pi[1,2,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(1==2)+gammawt[i]*(0==1)+gammawd[j,i]))^(-1/omega[i])))+
        pi[1,3,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(2==2)+gammawt[i]*(0==1)+gammawd[j,i]))^(-1/omega[i])))+
        pi[2,2,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(1==2)+gammawt[i]*(1==1)+gammawd[j,i]))^(-1/omega[i])))+
        pi[2,3,j,i]*(1-pweibull(5,omega[i],psi[i]*(exp(gammawe[i]*(2==2)+gammawt[i]*(1==1)+gammawd[j,i]))^(-1/omega[i])))
    }
  }
  return(re)
}
...
PGen12.I=function(ttox,teff,sigma12,score=matrix(c(20,50,100,0,30,60),2,3, byrow=TRUE),omega,psi,gammawe,gammawt,gammawd,A1,ncohort1,cohortsize1,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,startdose,dose.level){
  ndose<-dim(ttox)[2]
  ...
  for (i in 1: ncohort1){
    ...
    for(j in 1:cohortsize1){
      ...
      if(Y_E.1.temp==0)  Y_S.1.temp<-0 else {
        Y_S.1.temp<-pfs.gen_PGen12_weibull(omega[g.temp],psi[g.temp],gammawt[g.temp],gammawe[g.temp],gammawd[,g.temp],d,Y_E.1.temp,Y_T.1.temp)
      }
...
PGen12.II=function(Res1,score=matrix(c(20,50,100,0,30,60),2,3, byrow=TRUE),omega,psi,gammawe,gammawt,gammawd,A2,ncohort2,cohortsize2,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,t_l,t1,t2,rho=0.7,phi_S.lb=0.4,ps.lb=0.1,dose.level){
  pi<-Res1$pi
  ...
  if(earlystop[1]==1|length(acset.1)==0)  {
    ...
    for(i in 1:ncohort2){
      for(j in 1:cohortsize2){
        ...
        if(Y_E.12.temp==0)  Y_S.12.temp<-0 else Y_S.12.temp<-pfs.gen_PGen12_weibull(omega[g.temp],psi[g.temp],gammawt[g.temp],gammawe[g.temp],gammawd[,g.temp],d,Y_E.12.temp,Y_T.12.temp)
...
PGen12.III=function(Res2,score=matrix(c(20,50,100,0,30,60),2,3, byrow=TRUE),omega,psi,gammawe,gammawt,gammawd,A3,ncohort3,cohortsize3,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,t1,t2,phi_S.lb=0.4,ps.lb=0.5,dose.level){
  pi<-Res2$pi
  ...
  if(sum(earlystop)==3) {
    ...
    for(i in 1:ncohort3){
      for(j in 1:cohortsize3){
        ...
        if(Y_E.123.temp==0)  Y_S.123.temp<-0 else Y_S.123.temp<-pfs.gen_PGen12_weibull(omega[g.temp],psi[g.temp],gammawt[g.temp],gammawe[g.temp],gammawd[,g.temp],d,Y_E.123.temp,Y_T.123.temp)
...
Sim1<-function(Q){
  ...
  # pi
  ####weibull
  res_p<-gammawd_generate_weibull(pi,omega,gammawe,gammawt,sur.p)
  psi<-res_p$psi
  # psi
  ...
  sur.p.true=sur.p_PGen12_weibull(pi,omega,psi,gammawe,gammawt,gammawd)
  ttox
  teff
  u.true
  sur.p.true
  ...
  Res1<-PGen12.I(ttox,teff,sigma12,score,omega,psi,gammawe,gammawt,gammawd,A1,ncohort1,cohortsize1,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,startdose,dose.level)
  Res1$acset.1
  ...
  Res2<-PGen12.II(Res1,score,omega,psi,gammawe,gammawt,gammawd,A2,ncohort2,cohortsize2,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,t_l,t1,t2,rho=0.7,phi_S.lb=0.4,ps.lb=0.1,dose.level)
  ...
  ncohort3=10
  cohortsize3=3
  Res3<-PGen12.III(Res2,score,omega,psi,gammawe,gammawt,gammawd,A3,ncohort3,cohortsize3,targetT,targetE,cutoff.eli.T,cutoff.eli.E,p.g,t1,t2,phi_S.lb=0.4,ps.lb=0.5,dose.level)
  # Res3
...
clusterExport(cl, c('SIM1','Sim1','biv_PGen12','biv_PGen12_g','pmvnorm','gammawd_generate_weibull','uti_PGen12','uti_PGen12_g',
                    'sur.p_PGen12_weibull','PGen12.I','uti_benchmark_PGen12','get.boundary','pfs.gen_PGen12_weibull','adm',
                    'PGen12.II','rjmcmc.ETS','negative.log.likelihood.Y_ET','log.likelihood.Y_ET','negative.log.likelihood.Y_S',
                    'log.likelihood.Y_S','rtnorm','arms','log.likelihood.X_E','log.likelihood.sigma12',
                    'log.likelihood.Y_S_withoutconstraint','dtnorm','PGen12.III','adm_tox'))
